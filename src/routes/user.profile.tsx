...
function ProfileView({ admin = false }: { admin?: boolean }) {
  ...
  return (
    <DashboardLayout title={admin ? (lang === "ar" ? "الملف الشخصي - المدير" : "Admin Profile") : t("nav.profile", lang)}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between">
            <CardTitle className="font-display">{lang === "ar" ? "المعلومات الشخصية" : "Personal Info"}</CardTitle>
            {!editing ? (
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <Input dir="ltr" value={form.phone} disabled={!editing} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
              </div>
              <div className="space-y-1">
                <Label>{lang === "ar" ? "البريد" : "Email"}</Label>
                <Input type="email" dir="ltr" value={form.email} disabled={!editing} onChange={(e) => setForm({ ...form, email: e.target.value })} />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1 md:col-span-2">
                <Label>{lang === "ar" ? "نبذة" : "Bio"}</Label>
                <Textarea rows={3} value={form.bio} disabled={!editing} onChange={(e) => setForm({ ...form, bio: e.target.value })} />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader><CardTitle className="font-display text-base">{lang === "ar" ? "الإحصائيات" : "Stats"}</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="flex items-center gap-2 text-sm"><Car className="size-4 text-primary" />{lang === "ar" ? "الإعلانات" : "Listings"}</div>
              <div className="font-bold">8</div>
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="flex items-center gap-2 text-sm"><ShieldCheck className="size-4 text-success" />{lang === "ar" ? "صفقات مكتملة" : "Deals"}</div>
              <div className="font-bold">12</div>
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="flex items-center gap-2 text-sm"><Calendar className="size-4 text-warning" />{lang === "ar" ? "عضو منذ" : "Since"}</div>
...
export const Route = createFileRoute("/user/profile")({ component: () => <ProfileView /> });
